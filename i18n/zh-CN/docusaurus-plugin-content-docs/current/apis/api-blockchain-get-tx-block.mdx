...
:::note
  从 Zilliqa `V7.2.0` 开始，`header` 响应部分中包含一个额外的 `NumPages` 字段。此字段由 [GetTransactionsForTxBlockEx](api-transaction-get-txs-for-txblock-ex.mdx) 和 [GetTxnBodiesForTxBlockEx](api-transaction-get-txbodies-for-txblock-ex.mdx) 使用。
:::
...
| `method`  | string | 必填 | `"GetTxBlock"`                                           |
| `params`  | string | 必填 | 返回指定的 TX 区块高度。 如： `"1002353"` |